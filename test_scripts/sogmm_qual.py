...
if __name__ == "__main__":
    ...
    for f in args.frames:
        pcld_gt = 'pcd_' + str(f) + '_decimate_1_0.pcd'
        X_gt = o3d_to_np(o3d.io.read_point_cloud(pcld_gt))
        N_gt = X_gt.shape[0]

        for d in args.decimate:
            X_filepath = 'pcd_' + str(f) + '_decimate_' + \
                str(d).replace(".", "_") + '.pcd'
            ...
            for b in args.bandwidths:
                ...
                model_path = os.path.join(
                    tester.gmm_gpu_models, 'model_' + str(f) + '_' + str(d).replace(".", "_") + '_' + str(b).replace(".", "_") + '.pkl')
            ...
            metrics_file = 'metrics_' + str(f) + '_' + str(d).replace(".", "_") + '.csv'
            for j, b in enumerate(args.bandwidths):
                ...
                cprint('psnr %f support size %d' % (psnr, model.support_size_), 'green')
